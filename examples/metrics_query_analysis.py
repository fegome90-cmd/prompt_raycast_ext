...
async def main():
    ...
    print(f"Average Performance: {summary['avg_performance']:.2%}")
    print(f"Average Impact:    {summary['avg_impact']:.2%}")

    print("\n--- Grade Distribution ---")
    for grade, count in sorted(summary['grade_dist'].items()):
        bar = "█" * (count // max(1, max(summary['grade_dist'].values()) // 20))
