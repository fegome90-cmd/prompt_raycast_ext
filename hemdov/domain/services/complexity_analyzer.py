"""
Complexity Analyzer - Domain service for analyzing prompt complexity.

Analyzes prompt complexity using multi-dimensional scoring:
- Length analysis (40% weight)
- Technical term detection (30% weight)
...
class ComplexityAnalyzer:
    ...
    # Length score categories
    LENGTH_SCORE_SIMPLE = 0.0
    LENGTH_SCORE_MODERATE = 0.5
    LENGTH_SCORE_COMPLEX = 1.0

    # Technical term scoring
    TECHNICAL_TERM_MULTIPLIER = 0.5  # Each term adds 0.5, capped at 1.0
    STRUCTURE_PUNCTUATION_WEIGHT = 0.1  # Per punctuation mark
    ...
    def analyze(self, original_idea: str, context: str) -> ComplexityLevel:
        ...
        if total_length <= self.SIMPLE_MAX_LENGTH:
            length_score = self.LENGTH_SCORE_SIMPLE
        elif total_length <= self.MODERATE_MAX_LENGTH:
            length_score = self.LENGTH_SCORE_MODERATE
        else:
            length_score = self.LENGTH_SCORE_COMPLEX
